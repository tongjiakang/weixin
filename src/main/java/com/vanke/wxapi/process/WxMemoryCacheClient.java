...
public class WxMemoryCacheClient {
     ...
     * @return
     */
	public static AccessToken addAccessToken(String appId,AccessToken token){
		Map<String, AccessToken> accessTokenMap = WxApiClient.getAccessTokenCache();
		if(token != null){
			accessTokenMap.put(appId, token);
			CacheUtils.put(CommonContants.ACCESS_TOKEN,accessTokenMap);
		}
		return token;
	}
	 ...
	 * @return
     */
	public static AccessToken getSingleAccessToken(String appId){
		Map<String, AccessToken> accessTokenMap = WxApiClient.getAccessTokenCache();
