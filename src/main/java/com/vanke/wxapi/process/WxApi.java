...
import com.vanke.wxapi.util.WeiXinX509TrustManager;
...
public class WxApi {
	...
	//获取所有分组
	private static final String GET_GROUPINFO = "https://api.weixin.qq.com/cgi-bin/groups/get?access_token=%s";
	
	//新建分组
	private static final String CREATE_GROUP = "https://api.weixin.qq.com/cgi-bin/groups/create?access_token=%s";
	
	//修改分组名
	private static final String UPDATE_GROUP_NAME = "https://api.weixin.qq.com/cgi-bin/groups/update?access_token=%s";
	
	//移动分组
	private static final String MOVE_GROUP = "https://api.weixin.qq.com/cgi-bin/groups/members/batchupdate?access_token=%s";
	...
	//获取临时素材
	private static final String GET = "https://api.weixin.qq.com/cgi-bin/media/get?access_token=%s&media_id=%s";

	//新增永久图文素材
	private static final String ADD_NEWS = "https://api.weixin.qq.com/cgi-bin/material/add_news?access_token=%s";

	//上传图文素材里的图片
	private static final String CONTENT_IMG = "https://api.weixin.qq.com/cgi-bin/media/uploadimg?access_token=%s";
	...
	//获取素材列表
	private static final String BATCH_MATERIAL = "https://api.weixin.qq.com/cgi-bin/material/batchget_material?access_token=%s";

	//删除永久素材
	private static final String DEL_MATERIAL = "https://api.weixin.qq.com/cgi-bin/material/del_material?access_token=%s";

	//修改永久素材
	private static final String UPDATE_NEWS = "https://api.weixin.qq.com/cgi-bin/material/update_news?access_token=%s";

	//获取素材总数
	private static final String GET_MATERIALCOUNT = "https://api.weixin.qq.com/cgi-bin/material/get_materialcount?access_token=%s";
	...
	//新增永久图文素材
	public static String getAddNews(String token){
		return String.format(ADD_NEWS, token);
	}

	//获取正文图片素材链接
	public static String getContentImg(String token){
		return String.format(CONTENT_IMG, token);
	}
	
	//新增永久图文素材
	public static String getAddMaterial(String token,String type){
		return String.format(ADD_MATERIAL, token, type);
	}

	//获取临时素材
	public static String getGet(String token,String mediaId){
		return String.format(GET, token, mediaId);
	}
	...
	//获取分组接口
	public static String getGroupInfo(String token){
		return String.format(GET_GROUPINFO, token);
	}
	//新建分组
	...
	// 获取删除分组接口
	public static String getDeleteGroup(String token){
		return String.format(DELETE_GROUP, token);
	}
	//获取修改分组接口
	public static String getUpdateGroupName(String token){
		return String.format(UPDATE_GROUP_NAME, token);
	}
	
	//获取移动分组接口
	public static String getMoveGroup(String token){
		return String.format(MOVE_GROUP, token);
	}
	//获取粉丝信息接口
	...
	public static String getBatchMaterialUrl(String token){
		return String.format(BATCH_MATERIAL, token);
	}

	//删除素材接口
	public static String getDelMaterial(String token) {
		return String.format(DEL_MATERIAL, token);
	}

	//修改素材接口
	public static String getUpdateNews(String token) {
		return String.format(UPDATE_NEWS, token);
	}
	...
	public static AccessToken getAccessToken(Account account) {
		return getAccessToken(account.getAppid(),account.getEncodingAESKey());
	}
	 ...
	 * 返回media_id
	 */
	public static JSONObject uploadMedia(String uploadMediaUrl,String mediaUri) {
		...
		try {
			...
			// 根据内容类型判断文件扩展名
			// String fileExt = ".jpg";
			String fileName = mediaUri.substring(mediaUri.lastIndexOf("/")+1);
			
			return uploadMedia(uploadMediaUrl, fileName, contentType, meidaConn.getInputStream());
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * 返回media_id
	 */
	public static JSONObject uploadMedia(String uploadMediaUrl,String fileName,String contentType,InputStream inputStream) {
		...
		try {
			...
			description.put("introduction", fileName);
			outputStream.write(description.toJSONString().getBytes());
			...
			InputStream inputStreamIn = uploadConn.getInputStream();
			InputStreamReader inputStreamReader = new InputStreamReader(inputStreamIn, "utf-8");
	...
	/**
	 * 将正文中的图片上传到微信服务器中并返回替换过src 的正文
	 * @param content
	 * @return
	 * @throws Exception 
	 */
	public static String getContent(String token ,String content) throws Exception{
		Document doc = Jsoup.parseBodyFragment(content);
        Element body = doc.body();
        Elements elements = body.getElementsByTag("img");
        for (Element element : elements) {
        	 String url = element.attr("src");
        	 JSONObject jsonObject =  uploadMedia(getContentImg(token), url);
        	 if(jsonObject!=null){
     			if (0 == jsonObject.getIntValue( "errcode")) {
     				element.attr("src", jsonObject.getString("url"));
     			}
     			else {
     				throw new Exception("上传正文中的图片失败！错误码为："+jsonObject.getIntValue("errcode")+"错误信息为："+jsonObject.getString("errmsg"));
     			}
     		}else{
     			throw new Exception("上传正文中的图片地址错误");
     		}
		}
        return body.html();
	}
