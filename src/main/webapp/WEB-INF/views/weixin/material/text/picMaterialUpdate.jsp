...
<script type="text/javascript">
	function save() {
		...
		f.attr("action", "${ctx}/picMaterial/update");
		f.submit();
	}
	$(function() {
		$('#orgId').combotree({
		...
		$('#picMaterialAddForm').form({
			...
			success : function(result) {
				...
				if (result.success) {
					...
					self.parent.closeTab('图文编辑', "${param.title}");
				} else {
			...
			<input name="id" type="hidden" value="${template.id}"  />
				<table class="grid">
					...
					<tr>
						...
						<td class="tdContent2"><input name="templateName" type="text" maxlength="20" placeholder="请输入图文名称" value="${template.templateName}" class="easyui-validatebox"
					...
					<tr>
						...
						<td class="tdContent2"><select name="type" id="type">
								<option value="common" <c:if test="${template.type=='common'}">selected="selected"</c:if>>普通模板</option>
								<option value="cl" <c:if test="${template.type=='cl'}">selected="selected"</c:if>>超链接模板</option>
						</select></td>
					</tr>
				</table>
			</form>
