...
<c:if test="${fn:contains(sessionInfo.resourceList, '/sys/role/editPage')}">
...
<c:if test="${fn:contains(sessionInfo.resourceList, '/sys/role/grant')}">
	<script type="text/javascript">
		$.canGrant = true;
	</script>
</c:if>
...
<script type="text/javascript">
									...
									},
									{
										...
										formatter : function(value, row, index) {
											...
											if ($.canGrant) {
												str += $
														.formatString(
																'<a href="javascript:void(0)" onclick="grantFun(\'{0}\');" >授权</a>',
																row.id);
											}
											if (row.isdefault != 1) {
												str += '&nbsp;&nbsp;|&nbsp;&nbsp;';
		...
		parent.$.modalDialog({
			...
			buttons : [ {
				...
				handler : function() {
					...
					if(isValid){
						parent.$.messager.confirm('询问', '角色创建成功后将不能修改角色类型和组织机构，是否继续？', function(b) {
							if(b){
								f.submit();
							}
						});
	...
	function grantFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 500,
			height : 500,
			href : '${ctx}/sys/role/grantPage?id=' + id,
			buttons : [ {
				text : '保存',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
...
<body class="easyui-layout" data-options="fit:true,border:false">
	...
	<div id="toolbar" style="display: none;">
		<c:if test="${fn:contains(sessionInfo.resourceList, '/sys/role/addPage')}">
