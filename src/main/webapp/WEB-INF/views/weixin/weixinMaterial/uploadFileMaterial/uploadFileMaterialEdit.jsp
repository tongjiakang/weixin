...
<script type="text/javascript">
	$('#keyWordEditForm').form({
		url : '${ctx}/keyWord/update',
		onSubmit : function() {
			progressLoad();
	...
	$(function() {
		...
		$('#msgId').combogrid('setValue', "${keyWord.msgId}");
	});
</script>
			...
			<table class="grid">
				...
				<tr>
					...
					<td><input name="name" type="text" placeholder="请输入关键字"
						...
						value="${keyWord.name}" style="width: 200px; height: 21px;"></td>
				</tr>
				<tr>
					<td>消息类型:</td>
					<td><select name="msgType" id="msgType"
						class="easyui-combobox"
						data-options="editable:false,width:150,height:25,required:true">
							<option value="text"
								<c:if test="${keyWord.msgType=='text'}">selected</c:if>>文本消息</option>
							<option value="news"
								<c:if test="${keyWord.msgType=='news'}">selected</c:if>>图文消息</option>
					</select></td>
				</tr>
				<tr>
					<td>消息模板:</td>
					<td><input id="msgId" name="msgId" type="text"
